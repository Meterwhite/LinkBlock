...
{
    NSAssert([self.target isKindOfClass:[NSString class]], @"LinkBlock命令必须是objc字符串");
    
    if([self isLinkBlockCommand]){
        NSRegularExpression* regexOfNew = [NSRegularExpression regularExpressionWithPattern:@"[a-zA-Z_]+\\d*New" options:0 error:0];
        NSRange rangeOfMatch = [regexOfNew rangeOfFirstMatchInString:self.target options:0 range:NSMakeRange(0, [self.target length])];
        NSString* clzName = [self.target substringWithRange:rangeOfMatch];
        clzName = clzName.strTrimRight(@"New");
        Class clzz = NSClassFromString(clzName);
        if(clzz){
            return [[clzz alloc] init];
        }else if([[[self linkBlockCommandReflectList] allKeys] containsObject:self.target]){
            return [[self linkBlockCommandReflectList] objectForKey:self.target];
        }
        NSLog(@"DynamicLink Error:");
    }
    
    //直接响应方法
    SEL sel = NSSelectorFromString(self.target);
    if([target respondsToSelector:sel]){
        [target methodSignatureForSelector:sel];
        
        NSMethodSignature* sig = [NSMethodSignature methodSignatureForSelector:sel];
        if(sig.numberOfArguments>2){
            NSLog(@"DynamicLink Warning:%@,没有入参的方法调用!",self.target);
        };
        NSInvocation* invok = [NSInvocation invocationWithMethodSignature:sig];
        [invok invokeWithTarget:target];
        __block id reV;
        [self.class helpSwitchObjcType:sig.methodReturnType caseVoid:nil caseId:^{
            id val;
...
+ (NSMethodSignature *)helpSignatureForSelector:(SEL)aSelector
{
